...
const session = require('express-session'); // express-sessiton 미들웨어 -> 세션 관리
...
app.use(session({
  ...
  // name : 'session-cook' -> 세션 쿠키의 이름을 'session-cooke'로 지정
...
const upload = multer({
  ...
  // 업로드될 파일의 초대 크기 제한 -> 5MB(5 * 1024 * 1024byte)로 설정
...
app.get('/', (req, res, next) => {
  ...
  // app.set(주소, 라우터) -> 주소에 대한 GET요청이 올때 어떤 동작을 할지 적는 부분
